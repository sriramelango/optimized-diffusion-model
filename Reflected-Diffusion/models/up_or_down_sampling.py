...
# --- Added: upfirdn2d utility function ---
def upfirdn2d(x, f, pad=(0, 0)):
    """
    A simple PyTorch implementation of upfirdn2d for 2D images.
    x: (N, C, H, W)
    f: 1D or 2D FIR filter (as a torch tensor)
    pad: (pad0, pad1) or (left, right, top, bottom)
    """
    # If 1D kernel, make it 2D
    if f.dim() == 1:
        f = f[:, None] * f[None, :]
    f = f.to(x.device, dtype=x.dtype)
    N, C, H, W = x.shape
    # Pad
    ...
    f_flip = torch.flip(f, [0, 1]).unsqueeze(0).unsqueeze(0)
    x = F.conv2d(x, f_flip, stride=1, padding=0)
...
def downsample_2d(x, k=None, factor=2, gain=1):
    ...
    k = _setup_kernel(k) * gain
    p = k.shape[0] - factor
    return upfirdn2d(x, torch.tensor(k).to(x),
