...
def get_pc_sampler(sde, shape, predictor, corrector, denoiser, snr, 
    ...
    """
    Create a Predictor-Corrector (PC) sampler for diffusion models.

    This function supports context-freeze logic for context-aware generation:
    - For 3-channel input, channels 0 and 2 (context) are overwritten with their true values at every step.
    - Only the main channel (index 1) is generated by the diffusion process.

    Args:
        sde: The SDE object defining the forward process.
        shape: Shape of the sample to generate (B, 3, H, W).
        predictor: Predictor class for the PC sampler.
        corrector: Corrector class for the PC sampler.
        denoiser: Denoiser class for the PC sampler.
        snr: Signal-to-noise ratio for corrector.
        n_steps: Number of corrector steps per PC step.
        eps: Smallest time step for numerical stability.
        device: Device to run sampling on.

    Returns:
        pc_sampler: A function that generates samples given a model and context.
    """
    def pc_sampler(model, z=None, noise_removal_model=None, weight=0, class_labels=None, context_channels=None):
        """
        The PC sampler function for context-aware diffusion.

        Args:
            model: The trained score model.
            z: Optional initial noise for the main channel.
            noise_removal_model: Optional denoiser.
            weight: Guidance weight (if used).
            class_labels: Class labels for conditional generation.
            context_channels: [B, 2, H, W] tensor with true context (below, above).
        Returns:
            x: Generated samples [B, 3, H, W].
            n_steps: Number of function evaluations.
        """
        # Initialize x: only the main channel is noise, context is fixed
        if z is None:
            B, _, H, W = shape
            x = torch.rand(B, 3, H, W, device=device)  # Random noise for all channels
        else:
            x = z.clone()
        # Overwrite context channels at initialization
        if context_channels is not None:
            x[:,0,:,:] = context_channels[:,0,:,:]  # Channel 0: below (context)
            x[:,2,:,:] = context_channels[:,1,:,:]  # Channel 2: above (context)
        ...
        with torch.no_grad():
            ...
            for i in range(sde.N):
                ...
                if i < sde.N - 1:
                    ...
                    # Overwrite context channels at every step to ensure they are frozen
                    if context_channels is not None:
                        x[:,0,:,:] = context_channels[:,0,:,:]  # Channel 0: below (context)
                        x[:,2,:,:] = context_channels[:,1,:,:]  # Channel 2: above (context)
