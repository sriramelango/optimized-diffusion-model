...
class GTOHaloBenchmarker:
    ...
    def generate_samples(self) -> Tuple[np.ndarray, List[float]]:
        ...
        # Unnormalize times
        samples[:, 0] = samples[:, 0] * (max_shooting_time - min_shooting_time) + min_shooting_time
        samples[:, 1] = samples[:, 1] * (max_coast_time - min_coast_time) + min_coast_time
        samples[:, 2] = samples[:, 2] * (max_coast_time - min_coast_time) + min_coast_time
